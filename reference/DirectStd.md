...
This function computes the standardized direct effect of the independent
variable \\X\\ on the dependent variable \\Y\\ through mediator
variables \\\mathbf{m}\\ over a specific time interval \\\Delta t\\
using the first-order stochastic differential equation model's drift
matrix \\\boldsymbol{\Phi}\\ and process noise covariance matrix
\\\boldsymbol{\Sigma}\\.
...
The standardized direct effect of the independent variable \\X\\ on the
dependent variable \\Y\\ relative to some mediator variables
\\\mathbf{m}\\ is given by \$\$ \mathrm{Direct}^{\ast}\_{{\Delta t}\_{i,
j}} = \mathrm{Direct}\_{{\Delta t}\_{i, j}} \left(
\frac{\sigma\_{{x}\_{j}}}{\sigma\_{{y}\_{i}}} \right) \$\$ where
\\\boldsymbol{\Phi}\\ denotes the drift matrix, \\\sigma\_{{x}\_{j}}\\
and \\\sigma\_{{y}\_{i}}\\ are the steady-state model-implied standard
deviations of the state independent and dependent variables,
respectively, and \\\Delta t\\ the time interval.
