...
process noise covariance matrices \\\boldsymbol{\Sigma}\\ using the
Monte Carlo method.
...
Returns an object of class `ctmedmcphi` which is a list with the
following elements:
...
- output:
  ...
  A list of simulated draws. Each element contains a drift matrix and a
