...
#> Error in PBSSMOUFixed(R = R, path = path, prefix = prefix, n = n, time = time, : unused arguments (path = path, prefix = prefix)
```
...
#> Error in extract(object = boot, what = "phi"): could not find function "extract"
```


``` r
sigma <- extract(object = boot, what = "sigma")
#> Error in extract(object = boot, what = "sigma"): could not find function "extract"
```
...
#> Warning in parallel::mclapply(X = phi, FUN = .Med, delta_t = i, from = from, :
...
#> Warning in parallel::mclapply(X = phis, FUN = function(x, delta_t, from, : all
