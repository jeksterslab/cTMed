...
`vcov_phi_vec`` ``<-`` `[`vcov`](https://rdrr.io/r/stats/vcov.html)`(``fit``)``[``phi_varnames``, ``phi_varnames``]`
...
`delta`` ``<-`` `[`DeltaMed`](https://github.com/jeksterslab/cTMed/reference/DeltaMed.md)`(`\
`  phi ``=`` ``phi``,`\
...
`elapsed`` ``<-`` ``end`` ``-`` ``start`\
`elapsed`\
`#> Time difference of 0.8124709 secs`
...
\
`start`` ``<-`` `[`Sys.time`](https://rdrr.io/r/base/Sys.time.html)`(``)`\
`mc`` ``<-`` `[`MCMed`](https://github.com/jeksterslab/cTMed/reference/MCMed.md)`(`\
`  phi ``=`` ``phi``,`\
`  vcov_phi_vec ``=`` ``vcov_phi_vec``,`\
`  delta_t ``=`` ``delta_t``,`\
`  from ``=`` ``"x"``,`\
...
`elapsed`` ``<-`` ``end`` ``-`` ``start`\
`elapsed`\
`#> Time difference of 16.68126 mins`
