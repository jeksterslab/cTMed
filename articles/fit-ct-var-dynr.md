...
`sim`` ``<-`` `[`SimSSMOUFixed`](https://github.com/jeksterslab/simStateSpace/reference/SimSSMOUFixed.html)`(`\
`  n ``=`` ``n``,`\
`  time ``=`` ``time``,`\
`  delta_t ``=`` ``delta_t``,`\
`  mu0 ``=`` ``mu0``,`\
...
[`colnames`](https://rdrr.io/r/base/colnames.html)`(``data``)`` ``<-`` `[`c`](https://rdrr.io/r/base/c.html)`(``"id"``, ``"time"``, ``"x"``, ``"m"``, ``"y"``)`\
...
We use the `dynr` package to fit the continuous-time vector
autoregressive model.

\
[`library`](https://rdrr.io/r/base/library.html)`(`[`dynr`](https://dynrr.github.io/)`)`

### Prepare the Data

\
`dynr_data`` ``<-`` `[`dynr.data`](https://rdrr.io/pkg/dynr/man/dynr.data.html)`(`\
`  dataframe ``=`` ``data``,`\
`  id ``=`` ``"id"``,`\
`  time ``=`` ``"time"``,`\
`  observed ``=`` `[`c`](https://rdrr.io/r/base/c.html)`(``"x"``, ``"m"``, ``"y"``)`\
`)`

### Prepare the Initial Condition

\
`dynr_initial`` ``<-`` `[`prep.initial`](https://rdrr.io/pkg/dynr/man/prep.initial.html)`(`\
`  values.inistate ``=`` `[`c`](https://rdrr.io/r/base/c.html)`(`\
`    ``0``, ``0``, ``0`\
`  ``)``,`\
`  params.inistate ``=`` `[`c`](https://rdrr.io/r/base/c.html)`(`\
`    ``"mu0_1_1"``, ``"mu0_2_1"``, ``"mu0_3_1"`\
`  ``)``,`\
`  values.inicov ``=`` `[`matrix`](https://rdrr.io/r/base/matrix.html)`(`\
...
\
`dynr_measurement`` ``<-`` `[`prep.measurement`](https://rdrr.io/pkg/dynr/man/prep.measurement.html)`(`\
`  values.load ``=`` `[`diag`](https://rdrr.io/pkg/dynr/man/diag-character-method.html)`(``3``)``,`\
`  params.load ``=`` `[`matrix`](https://rdrr.io/r/base/matrix.html)`(`\
`    data ``=`` ``"fixed"``,`\
`    nrow ``=`` ``3``,`\
`    ncol ``=`` ``3`\
`  ``)``,`\
`  state.names ``=`` `[`c`](https://rdrr.io/r/base/c.html)`(``"eta_x"``, ``"eta_m"``, ``"eta_y"``)``,`\
`  obs.names ``=`` `[`c`](https://rdrr.io/r/base/c.html)`(``"x"``, ``"m"``, ``"y"``)`\
`)`

### Prepare the Dynamic Model

\
`dynr_dynamics`` ``<-`` `[`prep.formulaDynamics`](https://rdrr.io/pkg/dynr/man/prep.formulaDynamics.html)`(`\
...
\
`dynr_noise`` ``<-`` `[`prep.noise`](https://rdrr.io/pkg/dynr/man/prep.noise.html)`(`\
`  values.latent ``=`` `[`matrix`](https://rdrr.io/r/base/matrix.html)`(`\
`    data ``=`` ``0.2`` ``*`` `[`diag`](https://rdrr.io/pkg/dynr/man/diag-character-method.html)`(``3``)``,`\
`    nrow ``=`` ``3`\
`  ``)``,`\
...
`    nrow ``=`` ``3`\
`  ``)``,`\
`  values.observed ``=`` ``0.2`` ``*`` `[`diag`](https://rdrr.io/pkg/dynr/man/diag-character-method.html)`(``3``)``,`\
`  params.observed ``=`` `[`matrix`](https://rdrr.io/r/base/matrix.html)`(`\
`    data ``=`` `[`c`](https://rdrr.io/r/base/c.html)`(`\
...
In this example, we increase the maximum evaluations in the optimization
process and constrain the lower and upper bounds of the parameters for
the drift matrix.

\
`dynr_model`` ``<-`` `[`dynr.model`](https://rdrr.io/pkg/dynr/man/dynr.model.html)`(`\
`  data ``=`` ``dynr_data``,`\
...
`lb`` ``<-`` ``ub`` ``<-`` `[`rep`](https://rdrr.io/r/base/rep.html)`(``NA``, times ``=`` `[`length`](https://rdrr.io/r/base/length.html)`(``dynr_model``$``xstart``)``)`\
[`names`](https://rdrr.io/r/base/names.html)`(``ub``)`` ``<-`` `[`names`](https://rdrr.io/r/base/names.html)`(``lb``)`` ``<-`` `[`names`](https://rdrr.io/r/base/names.html)`(``dynr_model``$``xstart``)`\
`lb``[`\
...
\
`fit`` ``<-`` `[`dynr.cook`](https://rdrr.io/pkg/dynr/man/dynr.cook.html)`(`\
`  ``dynr_model``,`\
`  verbose ``=`` ``FALSE`\
`)`\
`#> [1] "Get ready!!!!"`\
...
`#> AIC = 429419.49`\
`#> BIC = 429676.34`\
`coefs`` ``<-`` `[`coef`](https://rdrr.io/r/stats/coef.html)`(``fit``)`\
`vcovs`` ``<-`` `[`vcov`](https://rdrr.io/r/stats/vcov.html)`(``fit``)`

### Extract Matrices from the Fitted Model to use in cTMed

\
`phi_names`` ``<-`` `[`c`](https://rdrr.io/r/base/c.html)`(`\
`  ``"phi_1_1"``, ``"phi_2_1"``, ``"phi_3_1"``,`\
`  ``"phi_1_2"``, ``"phi_2_2"``, ``"phi_3_2"``,`\
`  ``"phi_1_3"``, ``"phi_2_3"``, ``"phi_3_3"`\
`)`\
`sigma_names`` ``<-`` `[`c`](https://rdrr.io/r/base/c.html)`(`\
`  ``"sigma_1_1"``, ``"sigma_2_1"``, ``"sigma_3_1"``,`\
`  ``"sigma_2_1"``, ``"sigma_2_2"``, ``"sigma_3_2"``,`\
`  ``"sigma_3_1"``, ``"sigma_3_2"``, ``"sigma_3_3"`\
`)`\
`sigma_vech_names`` ``<-`` `[`c`](https://rdrr.io/r/base/c.html)`(`\
`  ``"sigma_1_1"``, ``"sigma_2_1"``, ``"sigma_3_1"``,`\
`  ``"sigma_2_2"``, ``"sigma_3_2"``,`\
`  ``"sigma_3_3"`\
`)`\
`theta_names`` ``<-`` `[`c`](https://rdrr.io/r/base/c.html)`(`\
`  ``"phi_1_1"``, ``"phi_2_1"``, ``"phi_3_1"``,`\
`  ``"phi_1_2"``, ``"phi_2_2"``, ``"phi_3_2"``,`\
`  ``"phi_1_3"``, ``"phi_2_3"``, ``"phi_3_3"``,`\
`  ``"sigma_1_1"``, ``"sigma_2_1"``, ``"sigma_3_1"``,`\
