...
lapply(
  ...
  FUN = function(i,
    ...
    if (requireNamespace("expm") && requireNamespace("simStateSpace")) {
        ...
        paste(text, "DirectCentralStd"),
        {
          ...
          for (m in seq_len(p)) {
            ...
            for (j in seq_len(p)) {
              for (i in seq_len(p)) {
                if (!(m == i || m == j || i == j)) {
                  expected[m] <- expected[m] + direct_std[i, j]
                }
              }
            }
          }
      ...
      testthat::test_that(
        ...
        {
          testthat::skip_on_cran()
          direct_central <- DirectCentralStd(
            phi = phi,
            sigma = sigma,
            delta_t = 1:5
          )
  ...
  text = "test-cTMed-direct-central-std",
  tol = 0.01
)
