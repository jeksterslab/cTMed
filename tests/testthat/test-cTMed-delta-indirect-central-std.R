## ---- test-cTMed-delta-indirect-central-std
lapply(
  X = 1,
  FUN = function(i,
                 text,
    ...
    testthat::test_that(
      paste(text, "DeltaIndirectCentralStd"),
      {
        ...
        delta_t <- 1:2
        output <- DeltaIndirectCentralStd(
          phi = phi,
          sigma = sigma,
    ...
    testthat::test_that(
      paste(text, "DeltaIndirectCentralStd diagonal sigma"),
      {
        testthat::skip_on_cran()
        phi <- matrix(
          data = c(
  ...
  text = "test-cTMed-delta-indirect-central-std",
  tol = 0.01
)
