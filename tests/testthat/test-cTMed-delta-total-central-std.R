## ---- test-cTMed-delta-total-central-std
lapply(
  X = 1,
  FUN = function(i,
                 text,
    ...
    testthat::test_that(
      paste(text, "DeltaTotalCentralStd"),
      {
    ...
    testthat::test_that(
      paste(text, "DeltaTotalCentralStd diagonal sigma"),
      {
        testthat::skip_on_cran()
        phi <- matrix(
          data = c(
  ...
  text = "test-cTMed-delta-total-central-std",
  tol = 0.01
)
