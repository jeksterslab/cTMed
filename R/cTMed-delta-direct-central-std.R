...
#' for the Standardized Direct Effect Centrality
...
#'
#' @details See [DirectCentralStd()] for more details.
#'
#' ## Delta Method
...
#'
#' @inheritParams DeltaDirectCentral
#' @inheritParams DeltaMedStd
...
#'     \item{fun}{Function used ("DeltaDirectCentralStd").}
...
#'     \item{est}{Estimated standardized direct effect centrality.}
...
#' @export
DeltaDirectCentralStd <- function(phi,
                                  sigma,
                                  vcov_theta,
                                  delta_t,
                                  sigma_diag = FALSE,
  ...
  stopifnot(
    idx == colnames(phi),
    is.matrix(sigma),
    all(dim(sigma) == dim(phi))
  )
  q <- if (sigma_diag) {
    length(.Vec(phi)) + length(diag(sigma))
  } else {
    length(.Vec(phi)) + length(.Vech(sigma))
  }
  stopifnot(
    is.matrix(vcov_theta),
    all(dim(vcov_theta) == c(q, q))
  )
  type <- "direct"
  delta_t <- sort(
    unique(
